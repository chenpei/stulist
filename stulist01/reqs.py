...
class CourseEditHandler(tornado.web.RequestHandler):
    def get(self, cou_sn):
        ...
        if cou is None:
            cou = dict(cou_sn='new', cou_no='', name='', notes='')
    ...
    def post(self, cou_sn):
        ...
        if cou_sn == 'new' :
            dal_create_course(cou_no, name, notes)
        else:
            dal_update_course(cou_sn, cou_no, name, notes)
...
def dal_get_course(cou_sn):
    with db_cursor() as cur : # 取得操作数据的游标，记为cur
        ...
        r = cur.fetchone()
        if r :
            return dict(cou_sn=r[0], cou_no=r[1], name=r[2], notes=r[3])


def dal_create_course(cou_no, name, notes):
    with db_cursor() as cur : # 取得操作数据的游标，记为cur
